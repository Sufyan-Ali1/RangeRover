import type { NationwideSupportData } from "../data/sections";

export default function NationwideSupport({ data }: { data: NationwideSupportData }) {
  return (
    <section className="w-full bg-white">
      <div className="mx-auto w-full max-w-[1728px] px-6 py-14 sm:px-10 xl:px-[101px] xl:py-16">
        ...
        {data.paragraphs.map((p, i) => (
          <p
            key={i}
            className={`text-[15px] leading-relaxed text-gray-700 ${i < data.paragraphs.length - 1 ? "mb-5" : ""}`}
          >
            {p}
          </p>
        ))}
